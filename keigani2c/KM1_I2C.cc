...
void KeiganMotor::begin()
{
  ...
  if (ioctl(_fd, I2C_SLAVE_FORCE, _address) == -1) {
   return;
  }    
...
bool KeiganMotor::write(uint8_t command, uint8_t *value, uint8_t value_len, bool response)
{
  ...
  uint8_t buffer_send[len-1];
  for(int i=1;i<len;i++){
    buffer_send[i-1]=tx_buf[i];
  }  

  if(i2c_smbus_write_i2c_block_data(_fd, command, len-1, (unsigned char*) buffer_send) == -1){
    return false;
  }
  ...
  usleep(I2C_WRITE_DELAY_MS);
  //From here it is missing yet
  
  //Wire.requestFrom((int)_address, RECV_DATA_ERROR_LEN);

  uint8_t rx_buf[RECV_DATA_ERROR_LEN];
  if(i2c_smbus_read_i2c_block_data(_fd, command, RECV_DATA_ERROR_LEN, (unsigned char*) rx_buf) == -1){
    return false;
  }

  //print_hexdump(rx_buf, RECV_DATA_ERROR_LEN);

  if (rx_buf[1] == RECV_DATA_ERROR)
  {
    error.cmd = rx_buf[4];
...
bool KeiganMotor::readRegister(uint8_t reg, uint8_t *value, uint8_t value_len)
{
  ...
  uint8_t buffer_send[CMD_REG_READ_REGISTER_LEN-1];
  for(int i=1;i<CMD_REG_READ_REGISTER_LEN;i++){
    buffer_send[i-1]=tx_buf[i];
  }  

  if(i2c_smbus_write_i2c_block_data(_fd, CMD_REG_READ_REGISTER, CMD_REG_READ_REGISTER_LEN-1, (unsigned char*) buffer_send) == -1){
    return false;
  } 
  usleep(I2C_READ_DELAY_MS);
  uint8_t rx_len = value_len + 7;
  if(i2c_smbus_read_i2c_block_data(_fd, CMD_REG_READ_REGISTER, rx_len, (unsigned char*) rx_buf) == -1){
    return false;
  }
  
  
  //print_hexdump(rx_buf, rx_len);

  if (crc16(rx_buf, rx_len) == 0)
  {
    if (rx_buf[1] == RECV_DATA_READ)
    {
...
bool KeiganMotor::readMotorMeasurement(void)
{
  ...
  append_crc16(tx_buf, len - 2);
  uint8_t buffer_send[CMD_READ_MOTOR_MEASUREMENT_LEN-1];
  for(int i=1;i<CMD_READ_MOTOR_MEASUREMENT_LEN;i++){
    buffer_send[i-1]=tx_buf[i];
  }  

  if(i2c_smbus_write_i2c_block_data(_fd, CMD_READ_MOTOR_MEASUREMENT, CMD_READ_MOTOR_MEASUREMENT_LEN-1, (unsigned char*) buffer_send) == -1){
    return false;
  } 
  usleep(I2C_READ_DELAY_MS);
  
  if(i2c_smbus_read_i2c_block_data(_fd, CMD_READ_MOTOR_MEASUREMENT, RECV_DATA_MOTOR_MEAS_LEN, (unsigned char*) rx_buf) == -1){
    return false;
  }  

  measurement = {false, 0, 0, 0};
  //print_hexdump(rx_buf, CMD_READ_MOTOR_MEASUREMENT_LEN);
  ...
  if (rx_buf[1] == RECV_DATA_MOTOR_MEAS)
  {
    ...
    else
    {
      ...
      error.code = ERROR_CODE_CRC_INVALID;
      error.info = 0xFF;
      //print_hexdump(rx_buf, RECV_DATA_MOTOR_MEAS_LEN);
    }
  }
...
bool KeiganMotor::readIMUMeasurement(void)
{
  ...
  append_crc16(tx_buf, len - 2);
  uint8_t buffer_send[CMD_READ_IMU_MEASUREMENT_LEN-1];
  for(int i=1;i<CMD_READ_IMU_MEASUREMENT_LEN;i++){
    buffer_send[i-1]=tx_buf[i];
  }  

  if(i2c_smbus_write_i2c_block_data(_fd, CMD_READ_IMU_MEASUREMENT, CMD_READ_IMU_MEASUREMENT_LEN-1, (unsigned char*) buffer_send) == -1){
    return false;
  } 
  usleep(I2C_READ_DELAY_MS);
  
  if(i2c_smbus_read_i2c_block_data(_fd, CMD_READ_IMU_MEASUREMENT, RECV_DATA_IMU_MEAS_LEN, (unsigned char*) rx_buf) == -1){
    return false;
  }  

  imu = {false, 0, 0, 0, 0, 0, 0, 0}; // response from KeiganMotor

  if (crc16(rx_buf, len) == 0)
  {
    if (rx_buf[1] == RECV_DATA_IMU_MEAS)
    {
      imu.isValid = true;
